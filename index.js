...
const not31 = ["chocolate", "vanilla", "strawberry"];
// console.log(is31Flavors(originalFlavors));
// console.log(is31Flavors(not31));
...
function filterByWord(array, flavor) {
  let newArray = array.filter((item) => item.includes(flavor));
  return newArray;
}

// let flavors = filterByWord(originalFlavors, "Vanilla");
// console.log(flavors);
...
function getAverageWordLength(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    newList.push(arr[i].split(" "));
  }

  for (let i = 0; i < newList.length; i++) {
    count += newList[i].length;
  }
...
function getRandomFlavors(arr1, arr2, arr3, arr4) {
  ...
  while (randomFlavors.length < 31) {
    let array = Math.floor(Math.random() * 4);
    let i = 0;
    randomFlavors.push(arrs[array][i]);
    i++;
  }
